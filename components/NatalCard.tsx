...
export const NatalCard: React.FC<Props> = ({ data, userData, lang, onReset }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-4 animate-[fadeIn_1.5s_ease-out]">
      <div className="glass-panel rounded-xl overflow-hidden shadow-[0_0_50px_-10px_rgba(88,28,135,0.4)] border-amber-500/10">
        ...
        <div className="relative p-8 text-center border-b border-white/10">
          ...
          <h3 className="text-purple-300 text-sm tracking-[0.3em] uppercase mb-2">{t.dailyGuidance} {userData.name}</h3>
          <h1 className="text-3xl md:text-5xl font-mystic text-amber-100 drop-shadow-lg leading-tight">
            {data.headline}
          </h1>
        </div>
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <StatBar label={t.love} value={data.stats.love} color="bg-rose-500" />
            <StatBar label={t.career} value={data.stats.career} color="bg-emerald-500" />
            <StatBar label={t.vitality} value={data.stats.vitality} color="bg-amber-500" />
          </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-4 border-t border-white/5">
            <div className="flex items-center gap-3">
               ...
               <div className="flex items-center gap-2">
                 ...
                 ></div>
                 <span className="font-mystic text-amber-100">{data.powerColor}</span>
               </div>
            </div>
            
            <button 
                onClick={onReset}
                className="text-xs text-purple-400 hover:text-amber-200 transition-colors uppercase tracking-widest border-b border-transparent hover:border-amber-200"
            >
                {t.readAnother}
            </button>
          </div>
